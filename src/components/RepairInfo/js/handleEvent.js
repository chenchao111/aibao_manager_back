...
export default {
  ...
  onSubmit() {
    this.form.data.forEach(item => {
      ...
      item['product_type'] = 501
      item['name'] = '快修服务'
      item['fullName'] = '快修服务'
    })
    // 校验非空
    if (!checkForm(this.form.data)) {
      this.$message.error('快修服务页面所有字段均为必填项，请确认是否全部填写完整！！')
      return
    }
    // 校验数字
    if (!checkNumber(this.form.data, 'original')) {
      this.$message.error('快修服务页面原价必须是数字，请确认填写是否规范！！')
      return
    }
    if (!checkNumber(this.form.data, 'cost')) {
      this.$message.error('快修服务页面销售价必须是数字，请确认填写是否规范！！')
      return
    }
    if (!checkNumber(this.form.data, 'discount')) {
      this.$message.error('快修服务页面成本价必须是数字，请确认填写是否规范！！')
      return
    }
    ...
    // 相同校验
    if (!checkSameMore(this.form.data, 'modelsId', 'partsId')) {
      this.$message.error('车型范围和部件不能完全重复，请确认后重新添加')
      return
    }
    if (this.isCreate) {
